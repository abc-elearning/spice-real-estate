...
### 2b. Scope Drift Detection

For each issue being reviewed, compare its stated file scope against actual changes.

1. Extract task's `files:` frontmatter from `.claude/epics/$EPIC_NAME/{issue}.md`:
   ```bash
   grep -A20 "^files:" .claude/epics/$EPIC_NAME/{issue}.md | grep "^  - " | sed 's/^  - //'
   ```
   If no `files:` key found → skip drift detection, output:
   ```
   📐 SCOPE CHECK: SKIPPED — No file scope defined for this task
   ```

2. Get actual changed files for the issue's commit range:
   ```bash
   git diff --name-only main..HEAD
   ```

3. Compare expected vs actual:
   - Extra files (in diff but NOT in task `files:`) → **DRIFT DETECTED**
     - Classify severity:
       - Same directory as a declared file → **minor**
       - Different component/directory → **major**
   - Declared files NOT touched by diff → check if AC requires them → **REQUIREMENTS MISSING**
   - All diff files within declared scope → **CLEAN**
...
4. Output format:
   ...
   📐 SCOPE CHECK: DRIFT DETECTED [{minor|major}]
   — Task "{title}" declared: {declared files}
   — Diff also touches: {extra files}

   📐 SCOPE CHECK: REQUIREMENTS MISSING
   — Declared files not touched: {files}
   — Review AC to confirm these were required
   ```

Edge cases:
- Git diff empty → output `CLEAN` (nothing changed = no drift)
- Shared config files (e.g., `package.json`, `*.json` in root) → do not flag as drift

### 2c. Fix-First Heuristic

After completing the Phase A prompt analysis, classify each finding before presenting to the developer.

**Classification rules:**

| Category | Examples | Action |
|---|---|---|
| AUTO-FIX | Formatting, trailing whitespace, missing newline at EOF, unused imports, obvious typos in strings | Apply silently, log result |
| ASK | Architecture changes, logic modifications, design decisions, dependency additions, public API changes | Present to user with options |

**For AUTO-FIX findings:**
- Apply the fix directly (edit the file)
- Log: `Auto-fixed: {description of what was fixed}`
