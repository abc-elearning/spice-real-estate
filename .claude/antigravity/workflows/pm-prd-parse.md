...
- **UI detection:** Scan the full PRD text for UI signals. Count matches from:
  - Keywords: `frontend`, `UI`, `screen`, `view`, `component`, `page`, `modal`, `dialog`, `form`, `button`, `dashboard`, `React`, `Vue`, `Svelte`, `Angular`, `HTML`, `CSS`, `Tailwind`
  - Path patterns: `src/components/`, `src/pages/`, `src/views/`, `app/screens/`
  - If ≥2 matches → set `UI_FEATURE=true`; if 0-1 matches → `UI_FEATURE=false` (skip coverage table)
  - Note: Only count matches in scope sections (not "Out of Scope")
...
##### Interaction State Coverage (if UI_FEATURE=true)

**Only if** `UI_FEATURE=true` (detected in Step 3), add to each UI-related task's Acceptance Criteria:
...
⚠️ "Empty states are features, not afterthoughts" — spec every cell.
```

Tasks for non-visual features (scripts, config, data processing) skip this table.

**Otherwise:** Skip — no coverage table added.
