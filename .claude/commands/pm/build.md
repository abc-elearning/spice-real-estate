...
- `--no-gate=<gate>` — auto-execute gate step without confirmation prompt; step still runs (can repeat: `--no-gate=plan-review --no-gate=epic-merge`)
...
Check if gate is in `--no-gate` list → **auto-execute** the step immediately without showing the gate prompt. This is equivalent to the user answering "yes" — the step STILL RUNS, only the confirmation prompt is bypassed. Do NOT skip execution.

Gate display format (only shown when gate is NOT in `--no-gate` list):
```
═══════════════════════════════════════
🚧 GATE: {step-name}
═══════════════════════════════════════
...
**CRITICAL:** `--no-gate` ≠ "skip". `--no-gate` = "yes" (auto-execute without asking).
