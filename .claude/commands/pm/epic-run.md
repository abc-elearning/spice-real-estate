...
Initialize self-regulation state:
```bash
risk_score=0
risk_reverts=0
risk_testfails=0
...
**Memory ingest (fire-and-forget):** After displaying task result, ingest summary into Memory Agent:
```bash
source .claude/scripts/pm/lifecycle-helpers.sh 2>/dev/null
if read_config_bool "memory_agent" "enabled" 2>/dev/null && \
   read_config_bool "memory_agent" "auto_ingest" 2>/dev/null; then
  ...
  SUMMARY_JSON=$(echo "Task #{task_num} ({task_name}) completed. Key changes: {brief_subagent_result}" | head -20 | jq -Rs .)
  curl -s --max-time 5 -X POST "http://${HOST}:${PORT}/ingest" \
    -H "Content-Type: application/json" \
    -H "X-Project-Root: $(pwd)" \
    -d "{\"text\": ${SUMMARY_JSON}, \"source\": \"epic-run-#{task_num}\", \"epic\": \"${epic_name}\"}" \
    >/dev/null 2>&1 || true
fi
```
...
**Memory ingest (fire-and-forget):** For each completed task in the parallel group, ingest its summary:
```bash
source .claude/scripts/pm/lifecycle-helpers.sh 2>/dev/null
if read_config_bool "memory_agent" "enabled" 2>/dev/null && \
   read_config_bool "memory_agent" "auto_ingest" 2>/dev/null; then
  ...
  [ "$HOST" = "null" ] || [ -z "$HOST" ] && HOST="localhost"
  [ "$PORT" = "null" ] || [ -z "$PORT" ] && PORT="8888"
  for task_num in {completed_task_nums}; do
    SUMMARY_JSON=$(echo "Task #${task_num} completed (parallel). Result: {task_summary}" | jq -Rs .)
    curl -s --max-time 5 -X POST "http://${HOST}:${PORT}/ingest" \
      -H "Content-Type: application/json" \
      -H "X-Project-Root: $(pwd)" \
      -d "{\"text\": ${SUMMARY_JSON}, \"source\": \"epic-run-#${task_num}\", \"epic\": \"${epic_name}\"}" \
...
#### 5d. Self-Regulation Heuristic

After each task (or parallel group) completes, scan the subagent result for risk signals:

```bash
# Scan task result for risk signals
result_text="{subagent_result}"

# Revert: git revert or git reset in output (+15% each occurrence)
revert_count=$(echo "$result_text" | grep -c "git revert\|git reset" || true)
[ "$revert_count" -gt 0 ] && risk_reverts=$((risk_reverts + revert_count)) && risk_score=$((risk_score + revert_count * 15))

# Test failures: FAIL/failed/error in test output (+10% per occurrence)
testfail_count=$(echo "$result_text" | grep -ci "\bFAIL\b\|failed\|test error" || true)
[ "$testfail_count" -gt 0 ] && risk_testfails=$((risk_testfails + testfail_count)) && risk_score=$((risk_score + testfail_count * 10))

# Scope drift: files modified outside task's files: list (+5% per file)
# Compare git diff --name-only against task's files: frontmatter
scope_files=$(git diff --name-only HEAD~1 HEAD 2>/dev/null || true)
task_files=$(grep -A20 "^files:" .claude/epics/${epic_name}/${task_num}.md | grep "^  - " | sed 's/^  - //' || true)
drift_count=0
for f in $scope_files; do
  echo "$task_files" | grep -qF "$f" || drift_count=$((drift_count + 1))
done
[ "$drift_count" -gt 0 ] && risk_scopedrift=$((risk_scopedrift + drift_count)) && risk_score=$((risk_score + drift_count * 5))
```

Display risk after each task:
```
⚡ Risk: {risk_score}% [revert:{risk_reverts} test-fail:{risk_testfails} scope:{risk_scopedrift}]
```

**Hard cap check — before starting each new task**, if `risk_score > 30`:

```
⚠️ Risk score {risk_score}% exceeds 30% threshold.
Reverts: {risk_reverts} (+{risk_reverts*15}%) | Test failures: {risk_testfails} (+{risk_testfails*10}%) | Scope drift: {risk_scopedrift} (+{risk_scopedrift*5}%)

Options:
  continue — reset score to 0, proceed with next task
  pause    — save state and exit (resume with /pm:epic-run {epic_name} --start-from <next_task>)
  abort    — stop epic-run, go to completion summary
```

- **continue**: Reset `risk_score=0`, `risk_reverts=0`, `risk_testfails=0`, `risk_scopedrift=0`, then proceed
- **pause**: Display resume instructions and exit
- **abort**: Go to Step 7 with partial results

> Note: Score at exactly 30% does NOT trigger pause — threshold is strictly `>30`.

#### 5e. Re-run Plan After Each Unit
