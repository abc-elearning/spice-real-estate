...
/pm:prd-rethink <feature_name> --mode=expand|hold|reduce|selective-expansion
```
...
2. Parse optional `--mode=expand|hold|reduce|selective-expansion` → set MODE (default: AUTO).
...
**Mental models:** Chesky "11-star experience" (dream big, dial back) · Jobs "1000 no's" (is this RIGHT?) · Bezos "working backwards" (start from user outcome) · Grove "strategic inflection" (12-month trajectory) · Bezos "one-way/two-way doors" (reversibility determines speed) · Munger "inversion" (avoid stupidity > seek brilliance) · Altman "leverage obsession" (what gives 10x for 2x effort?) · Chesky/Gebbia "design for trust" (remove friction, add confidence) · Rams "subtraction default" (when in doubt, remove) · Horowitz "wartime awareness" (is this peacetime or wartime decision?) · Grove "paranoid scanning" (what could kill us in 12 months?) · Altman "willfulness" (bet on ambition over safe choices).
...
**2D. Implementation Alternatives** — Bảng so sánh cách triển khai (luôn gồm 1 "minimal viable" + 1 "ideal architecture"):

| # | Approach | Effort | Risk | Pros | Cons | Reuses |
|---|----------|--------|------|------|------|--------|
| 1 | Minimal viable | S | Low | ... | ... | ... |
| 2 | Ideal architecture | L | Med | ... | ... | ... |

Hỏi: "Approach nào phù hợp nhất?" — Chờ phản hồi trước Phase 3.

**2E. Phân tích 3 Tầng:**
- **Tầng 1 — Tried-and-true:** Standard patterns, table stakes, mọi người đều làm
- **Tầng 2 — New-and-popular:** Trends, ecosystem buzz — với thái độ hoài nghi
- **Tầng 3 — First-principles:** Nơi conventional wisdom sai cho DỰ ÁN NÀY cụ thể
- **EUREKA check:** Flag `🔮 EUREKA: ...` khi Tầng 3 phát hiện insight bất ngờ
...
- **D) SELECTIVE EXPANSION** — Giữ scope gốc, cherry-pick từng expansion riêng. Cho: scope rõ nhưng có cơ hội mở rộng bên cạnh.

**Auto-defaults:** greenfield+chiến lược → EXPAND · rõ ràng → HOLD · chạm >3 systems → REDUCE · rõ ràng+cơ hội adjacent → SELECTIVE EXPANSION · user nói "go big" → EXPAND · user nói "MVP" → REDUCE.
...
#### Phân tích SELECTIVE EXPANSION

Liệt kê expansion opportunities từ phân tích Phase 2. Với mỗi cơ hội:

```
[N]. [Tên cơ hội] — Effort: [S/M/L], Risk: [Low/Med/High]. [1-line impact]
  → ✅ Include / ❌ Skip
```

Hỏi: "Những expansion nào nên include?" — User đánh dấu từng cơ hội. Nếu 0 cơ hội → fallback HOLD với ghi chú.
...
**Compress:** Phase 2 → one-paragraph summary · Phase 2D (Alternatives) → skip, note "user should compare approaches" · Phase 2E (3-Layer) → Layer 1 only · Phase 3 → recommendation + 1-sentence reasoning, skip sub-analyses · Phase 4 → 3 decisions as bullets · SELECTIVE EXPANSION → auto-default to HOLD.
