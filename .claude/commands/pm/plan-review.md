...
**Cognitive patterns:** Larson "state diagnosis" (technical, people, or process problem?) · McKinley "boring technology" (use proven tools unless 10x better) · Fowler "strangler fig" (wrap old, grow new, retire incrementally) · Brooks "essential vs accidental" (inherent complexity or self-inflicted?) · Beck "make change easy" (first make change easy, then make it) · Allspaw "failure is information" (every failure teaches — extract lesson) · Conway "org=architecture" (system mirrors team — check alignment) · Reilly "glue work" (value integration work, not just features)
...
Run silently, present compact summary:
- `git log --oneline -30 2>/dev/null` — recent activity
- `git stash list 2>/dev/null` — uncommitted work
- Grep TODO/FIXME in files listed in epic's Key files column
- Note recently-touched files (last 7 days)

Output: `📋 GIT AUDIT: {N} recent commits, {M} stashes, {K} TODOs/FIXMEs`

Graceful degradation: if any git command fails, skip that item silently. If git unavailable, skip entire section.

### Phase 0B: Retrospective Scan

`git log --oneline --all -- "**/plan-review.md" 2>/dev/null` — check for prior reviews on any branch.
- If found: read prior review, extract flagged areas. Output: `🔍 RETROSPECTIVE: {N} prior reviews found. Flagged areas: {list}`. In subsequent sections add note "⚡ Previously flagged — reviewing aggressively" for these areas.
- If none found: skip silently, proceed normally.
...
**0E. Implementation Alternatives** (skip if epic already discusses multiple approaches):

If epic proposes only one architecture/approach, present 2-3 alternatives:

| # | Approach | Effort | Risk | Pros | Cons | Reuses |
|---|----------|--------|------|------|------|--------|

Include ≥1 "minimal viable" + ≥1 "ideal architecture". Ask: "Epic chọn approach [X]. Đúng choice? Hay nên xét lại?"

If epic already discusses alternatives: acknowledge and validate choice.

**0F. Completeness Check:**

Principle: "AI coding compresses time 10-100x. Prefer full over shortcut when effort diff <2x."

For each task: evaluate if a "full" version exists vs the planned approach. If effort difference <2x → recommend full, note: "AI compression makes full viable". Track Lake Score: count of decisions choosing complete option / total decisions.

Add to Completion Summary: `Lake Score: {X}/{Y} ({Z}%) decisions chose complete option`
...
**Compress:** Section 1 → diagram + top 1 issue · Section 2 → critical gaps table only · Section 3 → skip if no DRY violations · Section 4 → coverage map only · Section 5 → skip unless PRD mentions performance · Phase 0A (Git Audit) → skip entirely · Phase 0B (Retrospective) → skip entirely · 0E (Alternatives) → skip if epic discusses alternatives · 0F (Completeness) → skip, note "check Lake Score post-review".
